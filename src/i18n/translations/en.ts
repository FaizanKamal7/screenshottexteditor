...
export const en: Translations = {
	...
	home: {
		...
		diffTitle: 'Accurate font matching, not AI-generated text',
		diffP1:
			"Generative image models redraw text as a plausible guess, which breaks down on small, crisp UI text. ScreenshotTextEditor uses AI-powered OCR and font matching instead, so your edit is rendered in the font that's actually in your screenshot.",
		diffP2:
		...
		privacyTitle: 'Private screenshot editing: auto-deleted uploads',
		privacyP:
		...
		useCasesTitle: 'What you can edit with a screenshot text editor',
		...
		aboutToolP2:
			"It runs entirely in the browser, so there's nothing to install and no design software required. Upload a screenshot or any image with text, click the line you want to change, type your replacement, and download the result — single images and batch localization alike.",
		aboutToolH3a: 'Why not a generative AI editor?',
		aboutToolP3:
			"Generative AI editors redraw text as a plausible-looking guess, which tends to fall apart on small, crisp UI fonts. This tool uses AI-powered OCR and font matching instead: it measures the font already in your image and rebuilds the text to match, as described in the OCR section above. Every match carries a visible confidence score, so if the result can't be trusted, it says so.",
		...
		aboutToolP5Pre:
			"Whether it's a quick typo fix, redacting a customer name before a demo, or a batch of localized App Store screenshots, the goal is to make the edit disappear rather than stand out, without needing a design background. Read more about",
		...
		faqs: [
			{
				...
				answer:
					'Open the ScreenshotTextEditor editor and drop in your screenshot or image. AI-powered OCR and font matching scan the image, detect every text run, and lets you click any line to rewrite it. Type your replacement, and the tool automatically matches the original font, size, weight, color, and anti-aliasing before rebuilding the image around your new text. Export it as a PNG or JPG in seconds, all from your browser.',
			},
			{
				question: 'How does an online image text editor work?',
				answer:
					'It uses AI-powered OCR and font matching in three steps: detect the text, match its font, then rebuild the line with your replacement. The OCR section above explains each step in detail.',
			},
			{
		...
		finalSubtitle: 'Upload a screenshot, change the text, and download the result.',
